...
ABSL_FLAG(std::string, config_path, "", "Path to the Inherent DataSource config XML file");
...
int main(int argc, char* argv[]) {
    ...
    absl::SetProgramUsageMessage(
        "Usage: inherent_riskprofile_cli [options]\n"
        "Evaluate inherent risk profiles\n\n"
        "Required flags:\n"
        "  --config_path=<path>    Path to Inherent DataSource config XML\n"
        "  --weight_path=<path>    Path to weight configuration file\n\n"
        "Optional flags:\n"
        "  --output_path=<path>    Path to output file (default: inherent_output.txt)"
    );
    ...
    if (has_error) {
        std::cerr << "\nUsage: inherent_riskprofile_cli --config_path=<path> --weight_path=<path> [--output_path=<path>]\n";
        return 1;
    }

    // Validate that files exist
    if (!std::filesystem::exists(config_path)) {
        std::cerr << "Error: Inherent config file not found: " << config_path << "\n";
        return 1;
    }
    ...
    try {
        LOG(INFO) << "Loading Inherent DataSource config from: " << config_path;
        LOG(INFO) << "Loading weight config from: " << weight_path;
        
        // Create evaluator with config and weight paths
        inherent::datasource::Evaluator evaluator(config_path, weight_path);

        LOG(INFO) << "Starting inherent risk profile evaluation...";

        // Run evaluation
        OperationStatus status = evaluator.evaluate();

        if (status == SuccessOperationStatus) {
            LOG(INFO) << "Inherent risk profile evaluation completed successfully";
            std::cout << "Successfully evaluated inherent risk profile\n";
            std::cout << "  Config: " << config_path << "\n";
            std::cout << "  Weight: " << weight_path << "\n";

            // Handle output
            if (output_path.empty()) {
                output_path = "inherent_output.txt";
            }
            ...
            ofs << "Inherent Risk Profile Evaluation Results\n";
            ofs << "========================================\n\n";
            ofs << "Input Files:\n";
            ...
            LOG(ERROR) << "Inherent risk profile evaluation failed";
            std::cerr << "Error: Inherent risk profile evaluation failed\n";
            return 1;
        }
